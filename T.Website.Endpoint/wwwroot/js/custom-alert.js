...
function OperationResult(response) {
    debugger;
    if (response.status === "success") {
        window.location.href = "https://localhost:7181" + response.redirectAction;
        $("#listTable").load(`${location.href} #listTable`);
...
function RemoveImage(id) {
    debugger;
    Swal.fire({
        title: "از حذف این عکس مطمئن هستید؟",
        text: "در صورت حذف این عکس از عکس های هتل حذف می شود ؟",
        icon: 'warning',
        showCancelButton: true,
        confirmButtonColor: '#3085d6',
        cancelButtonColor: '#d33',
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: `/Administration/HotelManagement/RemoveImage?id=${id}`,
                type: "post"
                , success: function (response) {
                    if (response.status === "success") {
                        toastr["success"](successOperationMessage)
...
function ShowAlertComment(id, action) {
    var title, text;
    if (action === "Delete") {
        title = deleteCommentTitleMessage;
        text = deleteCommentTextMessage;
    }
    if (action === "Confirmed") {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: `/Administration/CommentManagement/${action}?id=${id}`,
                type: "post"
                , success: function (response) {
                    if (response.status === "success") {
                        toastr["success"](successOperationMessage)
